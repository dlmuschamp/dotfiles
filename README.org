...
while a private window or a brand-new Zen profile worked fine.

*Root cause (confirmed 2026-09-07):* poisoned Outlook /site data/ in the
normal profile, not the Zen engine and not extensions. A fresh profile signs
in with Duo and stays signed in on a second cold tab. Failures in the normal
profile correlate with landing on =outlook.cloud.microsoft= after auth; working
sessions stay on =outlook.office.com=. OWA then calls =logoutRedirect()= against
inconsistent local state (MSAL / service workers / cookies). Brave never hit
this because Chromium keeps Microsoft's multi-host session coherent.

Two Firefox anti-tracking mechanisms also widen the margins around Outlook's
auth flow and are pinned in =zen/user.js= / applied by =zen-sso-fix=:

- *Bounce Tracking Protection* --- disabled in =user.js=.
- *Total Cookie Protection* --- cookie Allow, ETP-off, and
  =3rdPartyStorage^https://login.microsoftonline.com= grants so OWA's hidden
  AAD iframe can renew tokens.

Those permissions are not expressible as prefs alone, so =bin/zen-sso-fix=
writes them into =permissions.sqlite=. Zen must be fully closed when it runs.

*Recovery* --- when the sign-out loop is active, use =--deep= (also what
=zen-outlook --recover= runs). A shallow Outlook-only clear can leave a
half-valid AAD session that still fails after Stanford login; clearing
Microsoft/AAD cookies forces the same full SSO + Duo path that works in a
fresh profile:

#+BEGIN_SRC sh
zen-sso-fix --deep      # clears Microsoft/Outlook state; costs one Duo
zen-sso-fix             # milder: Outlook-only; keeps AAD if you only need grants
#+END_SRC
...
zen-outlook --recover    # quit Zen, zen-sso-fix --deep, reopen mail
#+END_SRC

After recovery, verify with a *second* cold tab to the same URL. Do not
disable cookies for Outlook --- that breaks SSO entirely.
