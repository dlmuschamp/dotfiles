...
;; BlexMono = IBM Plex Mono (Nerd Font).
;; Emacs GUI pts render larger than Alacritty for the same number — at
...
(setq doom-font (font-spec :family luciano/ui-font-family
      ...
      doom-variable-pitch-font (font-spec :family "IBM Plex Sans"
                                          :size (/ luciano/ui-font-height 10.0))
      doom-big-font (font-spec :family luciano/ui-font-family :size 16.0)
      doom-serif-font (font-spec :family "IBM Plex Serif"
                                 :size (/ luciano/ui-font-height 10.0)))
(add-to-list 'default-frame-alist
             `(font . ,(format "%s-%s"
                               luciano/ui-font-family
                               (/ luciano/ui-font-height 10.0))))

(defun luciano/confirm-ui-font ()
  "Apply calibrated UI font (matches Alacritty 14 by line density)."
  (when (display-graphic-p)
    (set-face-attribute 'default nil
                        :family luciano/ui-font-family
                        :height luciano/ui-font-height
                        :width 'normal
                        :weight 'regular)
...
(add-hook 'doom-after-init-hook #'luciano/confirm-ui-font 90)
...
;; Official Emacs splash + a short personal line (edit the defvar, not a slogan).
(setq fancy-splash-image
      (expand-file-name "banners/emacs-splash.png" doom-user-dir)
      +dashboard-anchor '(center . center)
      +dashboard-banner-vertical-padding '(1 . 1))

;; Short line under the logo.
(defvar luciano/dashboard-tagline "github.com/dlmuschamp"
  "Centered subtitle under the dashboard splash.")

(defun luciano/dashboard-ascii-banner ()
  "TTY fallback; GUI uses `luciano/dashboard-widget-banner'."
  (propertize "EMACS\n" 'face '+dashboard-banner))
(setq +dashboard-ascii-banner-fn #'luciano/dashboard-ascii-banner)

(defun luciano/dashboard--insert-centered (text &optional face)
  "Insert TEXT centered on the current line, then a newline."
  (when (and text (not (string-empty-p text)))
    (let* ((face (or face '+dashboard-banner))
           (line (propertize text 'face face))
           (width (string-width line))
           (prefix (propertize " " 'display
                               `(space :align-to (- center ,(/ width 2))))))
      (insert prefix line "\n"))))

(defun luciano/dashboard-widget-banner ()
  "Draw a centered Emacs splash with an optional personal tagline."
  (let* ((path fancy-splash-image)
    ...
    (cond
     ((and (display-graphic-p) path (file-readable-p path))
      (let* ((img (create-image path 'png nil :width 480))
             (prefix (propertize " " 'display
                                 `(space :align-to (- center (0.5 . ,img)))))
             (line (concat prefix (propertize " " 'display img) "\n")))
        (insert line)))
     (t
      (let* ((banner (funcall +dashboard-ascii-banner-fn))
             (width (+dashboard-maxlen banner))
             (prefix (propertize " " 'display
                                 `(space :align-to (- center ,(/ width 2))))))
        (add-text-properties 0 (length banner)
                             `(line-prefix ,prefix wrap-prefix ,prefix)
                             banner)
        (insert banner)
        (unless (string-suffix-p "\n" banner) (insert "\n")))))
    ;; Small breath between logo and label.
    (insert (propertize "\n" 'display '(space :height 0.6)))
    (luciano/dashboard--insert-centered luciano/dashboard-tagline
                                        '+dashboard-banner)
    (when (> bot-pad 0)
      (insert (propertize "\n" 'display `(space :height ,bot-pad))))))

;; Replace the stock banner widget (user config loads after the module).
(setq +dashboard-functions
      '(luciano/dashboard-widget-banner
...
;;; ==========================================
;;; THEME — track Aether/Omarchy (bg + hues)
;;; ==========================================
;; Aether writes ~/.local/state/omarchy/current/theme/colors.toml (and
;; neovim.lua / alacritty.toml). Doom mirrors that palette — including
;; background — so terminal, nvim, and Emacs feel like one system.
;;
;; Flat canvas (solaire off). Prefer bright_* ANSI slots when the base
;; hue is too dim to read on dark themes.
(defvar luciano/omarchy-colors nil
  "Alist of color keys from the active Omarchy/Aether colors.toml.")

(defvar luciano/omarchy-colors-file
  (expand-file-name
   "omarchy/current/theme/colors.toml"
   (or (getenv "XDG_STATE_HOME")
       (expand-file-name "~/.local/state")))
  "Canonical Omarchy/Aether colors.toml path.")

(defvar luciano/omarchy-colors-watch nil
  "file-notify descriptor for live Aether reloads.")
...
(defun luciano/load-omarchy-colors ()
  ...
  (let ((file luciano/omarchy-colors-file)
        colors)
    (unless (file-readable-p file)
      (setq file (expand-file-name "~/.config/aether/theme/colors.toml")))
    (unless (file-readable-p file)
...
(defun luciano/apply-omarchy-theme-faces (&rest _)
  "Restyle Doom to match the active Aether/Omarchy palette (incl. background)."
  (luciano/load-omarchy-colors)
  (let* ((bg      (or (luciano/omarchy-color "background")
                      (luciano/omarchy-color "bg")
                      "#1f1f28"))
         (bg-alt  (or (luciano/omarchy-color "lighter_background")
                      (luciano/omarchy-color "lighter_bg")
                      (luciano/omarchy-color "selection")
                      "#35353e"))
         (chrome  (or (luciano/omarchy-color "dark_background")
                      (luciano/omarchy-color "dark_bg")
                      bg))
         (fg      (or (luciano/omarchy-color "foreground")
                      (luciano/omarchy-color "fg")
                      "#dcd7ba"))
         (fg-dim  (or (luciano/omarchy-color "dark_foreground")
                      (luciano/omarchy-color "dark_fg")
                      "#a5a18c"))
         (accent  (or (luciano/omarchy-color "accent")
                      (luciano/omarchy-color "bright_blue")
         ...
         (magenta (or (luciano/omarchy-color "bright_magenta")
                      ...
                      "#938aa9"))
         (orange  (or (luciano/omarchy-color "orange") "#cc5d5f"))
         (region-bg (or (luciano/omarchy-color "selection")
                        (luciano/omarchy-color "selection_background")
                        bg-alt))
         (region-fg (or (luciano/omarchy-color "selection_foreground") fg)))
    (custom-set-faces!
    ...
    (message "Aether theme → bg %s / fg %s" bg fg)))

(defun luciano/watch-omarchy-theme ()
  "Reload Doom faces when Aether/Omarchy swaps the current theme.
Watch the parent `current/` dir — Omarchy replaces `theme/` atomically."
  (when (fboundp 'file-notify-add-watch)
    (let ((dir (expand-file-name
      ...
      (when (file-directory-p dir)
        (when luciano/omarchy-colors-watch
          (ignore-errors (file-notify-rm-watch luciano/omarchy-colors-watch)))
        (setq luciano/omarchy-colors-watch
              (file-notify-add-watch
               dir
               '(change)
               (lambda (_event)
                 (run-with-idle-timer 0.35 nil #'luciano/apply-omarchy-theme-faces))))))))
...
(add-hook 'doom-after-init-hook #'luciano/watch-omarchy-theme)
